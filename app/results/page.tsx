...
import { DynamicScoreCard, LoadingSpinner, CardGridSkeleton, TableSkeleton } from '../../components/LazyComponents';

// Recommendation Card Component with lazy loading
...
// Assessment Detail Component with lazy loading
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="p-6 max-w-7xl mx-auto">
              ...
              <div>
                ...
                <div className="text-slate-600 space-y-1 sm:space-y-0">
                  <p className="sm:inline">
                    <span className="font-medium">นักเรียน:</span> {assessment.studentName}
                  </p>
                  <p className="sm:inline sm:ml-4">
                    <span className="font-medium">ห้อง:</span> {currentClassroom.name}
                  </p>
                  <p className="sm:inline sm:ml-4">
                    <span className="font-medium">วันที่ประเมิน:</span> {new Date(assessment.completedDate || '').toLocaleDateString('th-TH')}
                  </p>
                </div>
              </div>
        ...
        {/* Score Cards with Lazy Loading */}
        <Suspense fallback={<CardGridSkeleton cols={3} rows={2} />}>
              ...
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <p className="text-slate-900 text-sm sm:text-base">
                  {impactQuestions.mainQuestion.options.find(
                    o => o.value === assessment.impactResponses?.hasProblems
                  )?.label}
                </p>
              </div>

              {assessment.impactResponses.hasProblems > 0 && (
                <>
                  {assessment.impactResponses.duration !== undefined && (
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      ...
                      <p className="text-slate-900 text-sm sm:text-base">
                        {impactQuestions.followUpQuestions.find(q => q.id === 'duration')?.options.find(
                          o => o.value === assessment.impactResponses?.duration
                        )?.label}
                      </p>
                    </div>
                  )}

                  {assessment.impactResponses.distress !== undefined && (
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      ...
                      <p className="text-slate-900 text-sm sm:text-base">
                        {impactQuestions.followUpQuestions.find(q => q.id === 'distress')?.options.find(
                          o => o.value === assessment.impactResponses?.distress
                        )?.label}
                      </p>
                    </div>
                  )}

                  {assessment.impactResponses.impactFriends !== undefined && (
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      ...
                      <p className="text-slate-900 text-sm sm:text-base">
                        {impactQuestions.followUpQuestions.find(q => q.id === 'impactFriends')?.options.find(
                          o => o.value === assessment.impactResponses?.impactFriends
                        )?.label}
                      </p>
                    </div>
                  )}

                  {assessment.impactResponses.impactLearning !== undefined && (
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      ...
                      <p className="text-slate-900 text-sm sm:text-base">
                        {impactQuestions.followUpQuestions.find(q => q.id === 'impactLearning')?.options.find(
                          o => o.value === assessment.impactResponses?.impactLearning
                        )?.label}
                      </p>
                    </div>
                  )}

                  {assessment.impactResponses.burdenOthers !== undefined && (
                    <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      ...
                      <p className="text-slate-900 text-sm sm:text-base">
                        {impactQuestions.followUpQuestions.find(q => q.id === 'burdenOthers')?.options.find(
                          o => o.value === assessment.impactResponses?.burdenOthers
                        )?.label}
                      </p>
                    </div>
                  )}
...
const ResultsPage: React.FC = () => {
  ...
  if (selectedAssessment) {
    return (
      <Suspense fallback={<LoadingSpinner message="กำลังโหลดรายละเอียด..." />}>
        <AssessmentDetail 
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {filteredAssessments.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            <Suspense fallback={<TableSkeleton rows={filteredAssessments.length} />}>
